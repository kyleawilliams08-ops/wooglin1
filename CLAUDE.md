...
- **CTP** (Closest to the Pin): `src/lib/ctpActions.ts` (claimCtp — any event participant self-claims via ConfirmForm on /matches; takeover replaces holder; claims lock once every match in the round is final, admins excepted) + CTP section on the round admin page (`/admin/events/[id]/rounds/[roundId]`: add/remove holes validated against the round's side, set/clear holder for rulings). Holder strip per CTP hole on /matches round cards (🏆 once round done); feed kind 'ctp' posts takeovers; `ctp_holes` realtime-published + in LiveRefresher.
