...
- **Sign out is `signOut({ scope: "local" })`** — Supabase defaults to `global`, which revokes the player's sessions on EVERY device (sign out on a laptop → phone logged out mid-round). Never call bare `signOut()`.
...
ctp_holes(id, round_id, hole_number, holder_participant_id?, holder_set_at, holder_set_by, stake?, bet_id?)  -- unique (round,hole); CTP holes; current holder wins (king-of-the-hill claims); optional per-player stake settles via "Post to ledger" (round admin page) into an already-closed group bet (everyone in the round's matchups in, holder wins, year = CURRENT calendar year to match the wizard/ledger filters) — bet_id blocks double-posting
ctp_claims(id, ctp_id, participant_id, claimed_by, created_at)  -- claim-chain history (every takeover incl. commissioner calls); /matches strip shows "JC → Shoops → Kyle" when >1
```
...
- `src/lib/scoring.ts` — `assertCanScore` (admin OR match participant) + score writes. **Several phones score one match at once, so writes are column-scoped** (`writeHoleColumns`: update just the named columns → insert if no row → retry on unique-violation) — never read-modify-write a whole `hole_scores` row, it loses the other phone's score. Writes THROW on failure. Full-card saves are merge-safe (`cardPatch`, tested): a blank cell = no change, never "clear", so a stale card can't wipe partners' holes — clear a score from the hole-by-hole scorer. `HoleByHole` keeps an **outbox** of unconfirmed writes: re-applied over every refresh, red "not saved" banner, auto-retry on `online` + every 15s. `LiveRefresher` also re-pulls on `visibilitychange`/`online` (a pocketed PWA has a dead socket). `liveScoring.sim.test.ts` replays whole matches + a week of bets through the pure engines.
...
Year-scoped side bets (NOT event-tied): `bets` + `bet_participants`. Lifecycle: propose via wizard (/bets/new, roster-limited, live IMMEDIATELY — no acceptance) → any participant closes with a ConfirmForm tap (side buttons / group winner / push) → losing side can PROTEST closed bets, any participant can protest a push (protested = frozen out of ledger, `protested_from` remembers restore target) → resolve: protester withdraws, winners concede (void), or admin dismisses/reopens. Creator/admin can cancel open bets. Settle/cancel/protest put the expected status in the UPDATE's WHERE clause and check the affected row count, so two people settling at once can't interleave winners (the loser of the race gets "already settled"). `/bets` = "Open Action" panel (your open/protested bets, pinned on top) + Propose button + a **Ledger / Activity tab toggle** (`?tab=`): Ledger = everyone's net with a player-name search (`LedgerList`), Activity = all bet cards (open + settled) filtered via a funnel bottom-sheet (`BetFilter`, `?who=`/`?status=`). Tap a ledger row → `/bets/player/[id]` audit trail (`PlayerBetList`, searchable by bet name). Home shows a Your-open-bets card + your net. Feed posts on propose/close/protest.
